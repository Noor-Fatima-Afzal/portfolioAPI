...
class CVRAGPipeline:
    def __init__(self) -> None:
        ...
        print("=" * 60)
        print("🔧 GROQ API Pipeline Initialization")
        print("=" * 60)
        print(f"✓ GROQ_API_KEY: {'✓ SET' if self.api_key else '✗ MISSING'}")
        print(f"✓ GROQ_MODEL: {self.model_name}")
        print(f"✓ CV_PATH: {self.cv_path}")
        print(f"✓ CV File exists: {'✓ YES' if os.path.exists(self.cv_path) else '✗ NO'}")
        print("=" * 60)
        
        # Initialize Groq client
        if not self.api_key:
            self._init_error = "GROQ_API_KEY is not set. Please set it in .env or environment variables."
        ...
        try:
            ...
            print(f"✅ Knowledge base built: {self.chunk_count} chunks loaded")
        except FileNotFoundError as e:
            self._init_error = str(e)
            print(f"❌ ERROR: {self._init_error}")
        except Exception as e:
            self._init_error = f"Knowledge base error: {str(e)}"
            print(f"❌ ERROR: {self._init_error}")
    ...
    def answer_question(self, question: str, top_k: int = 4) -> Tuple[str, int]:
        if not self._client:
            raise RuntimeError(f"API not ready: {self._init_error or 'Unknown error'}")
